...
for ver in 9.0 8.0; do
  ...
  # Read pkgver list via FD 3 so the while loop doesn't shadow stdin —
  # the router needs the parent's stdin (MCP JSON-RPC) after `exec`.
  while IFS= read -r pkgver <&3; do
    cand="$base/$pkgver/router/$rid/rhino-mcp-router"
    if [ -x "$cand" ]; then
      exec "$cand" "$@"
    fi
  done 3< <(ls -1 "$base" 2>/dev/null | sort -r)
done
